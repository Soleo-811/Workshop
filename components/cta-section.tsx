...
export default function CTASection() {
  const [formData, setFormData] = useState({
    ...
    company: "",
    email: "",
    // THÊM TRƯỜNG MỚI: Mã giới thiệu (tùy chọn)
    referrerCode: "",
    package: "",
    quantity: 2,
  })
        ...
        {!showSuccess ? (
          <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-2xl space-y-4 mb-10 text-gray-800">
            ...
            <div className="space-y-4">
              ...
              {/* PHẦN ĐÃ SỬA: Đặt Email và Mã giới thiệu vào cùng một grid */}
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="email"
                    name="email"
                    placeholder="Nhập Email của Anh Chị"
                    value={formData.email}
                    onChange={handleChange}
                    className={`w-full p-3 border rounded-lg focus:outline-none focus:ring-2 ${
                      errors.email ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:ring-red-500"
                ...
                {/* TRƯỜNG MÃ GIỚI THIỆU MỚI */}
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2 text-left">Mã giới thiệu (Nếu có)</label>
                  <input
                    type="text"
                    name="referrerCode"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option value="">Chọn Gói Đăng Ký</option>
                    <option value="3-day">Gói THỰC CHIẾN (3 Ngày)</option>
                    <option value="2-day">Gói TƯ DUY (2 Ngày)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2 text-left">Số lượng học viên *</label>
                  <input
                    type="number"
                    name="quantity"
                    placeholder="Nhập số lượng người tham dự dự kiến"
                    min="2"
                    value={formData.quantity}
                    onChange={handleChange}
                    className={`w-full p-3 border rounded-lg focus:outline-none focus:ring-2 ${
          ...
          <div className="bg-white p-8 rounded-2xl shadow-2xl mb-10 border-4 border-green-500">
            <h3 className="text-4xl font-black mb-4 text-center text-green-500">✓ ĐĂNG KÝ THÀNH CÔNG</h3>
            <p className="text-lg text-gray-700 mb-6 text-center">
              Cảm ơn Anh chị đã điền thông tin. Bộ phận Hỗ trợ Lớp sẽ liên lạc trong 24h qua Zalo hoặc cuộc gọi đến số điện thoaị đăng ký. Anh chị vui lòng check Email
            </p>
